package com.github.stefvanschie.inventoryframework.nms.v1_8_8;

import com.github.stefvanschie.inventoryframework.abstraction.AnvilInventory;
import com.github.stefvanschie.inventoryframework.adventuresupport.TextHolder;
import com.github.stefvanschie.inventoryframework.nms.v1_8_8.util.TextHolderUtil;
...
public class AnvilInventoryImpl extends AnvilInventory {
    ...
    public void openInventory(@NotNull Player player, @NotNull TextHolder title,
        ...
        entityPlayer.playerConnection.sendPacket(new PacketPlayOutOpenWindow(id, "minecraft:anvil", message));
    ...
    public void sendItems(@NotNull Player player, @Nullable org.bukkit.inventory.ItemStack[] items) {
        List<ItemStack> nmsItems = Arrays.asList(
    ...
    public void clearResultItem(@NotNull Player player) {
        sendResultItem(player, new ItemStack(Blocks.AIR));
    }
    ...
    public void clearCursor(@NotNull Player player) {
        getPlayerConnection(getEntityPlayer(player)).sendPacket(new PacketPlayOutSetSlot(-1, -1, new ItemStack(Blocks.AIR)));
    }
    ...
    private class ContainerAnvilImpl extends ContainerAnvil {
        ...
        /**
         * Field for accessing the entity human field
         */
        @NotNull
        private final Field entityhumanAccessField;
...
//                ContainerAccess.at(entityPlayer.getWorld(), new BlockPosition(0, 0, 0)));
            ...
            try {
                repairInventoryField = ContainerAnvil.class.getDeclaredField("h");
                repairInventoryField.setAccessible(true);

                resultInventoryField = ContainerAnvil.class.getDeclaredField("g");
                resultInventoryField.setAccessible(true);

                entityhumanAccessField = ContainerAnvil.class.getDeclaredField("m");
                entityhumanAccessField.setAccessible(true);
        ...
        public CraftInventoryView getBukkitView() {
            if (bukkitEntity == null) {
                CraftInventory inventory = new CraftInventoryAnvil(getRepairInventory(), getResultInventory()) {
                    @NotNull
                    @Contract(pure = true)
                    @Override
        ...
        private EntityHuman getEntityHuman() {
            try {
                return (EntityHuman) entityhumanAccessField.get(this);
            } catch (IllegalAccessException exception) {
                throw new RuntimeException(exception);
            }
        }
